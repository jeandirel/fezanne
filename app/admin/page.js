            ...
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="rounded-[2rem] bg-black/20 border-white/10 p-6 shadow-xl relative overflow-hidden">
                ...
                <form onSubmit={handleSave} className="space-y-6">
                  ...
                  <div>
                    <Label className="text-xs uppercase tracking-wider text-amber-200/70 flex items-center gap-1.5"><ImageIcon className="w-3 h-3" /> Image personnalisée</Label>
                    <div className="mt-2 flex gap-2">
                      <label className="flex-shrink-0 cursor-pointer">
                        ...
                        />
                        <div className="h-11 px-4 rounded-xl bg-amber-500/20 border border-amber-500/40 hover:bg-amber-500/30 flex items-center gap-2 text-amber-200 text-xs font-medium transition-colors">
                      ...
                      <Input
                        ...
                        className="rounded-xl bg-black/30 border-white/10 h-11 text-xs font-mono focus:ring-amber-500/20 flex-1"
                      />
                    </div>
                    {formImage && (
                      <div className="mt-2 flex items-center gap-2">
                        <img src={formImage} alt="preview" className="w-10 h-10 rounded-lg object-cover border border-white/20" onError={e => e.target.style.display='none'} />
                        <span className="text-[10px] text-amber-200/60 font-mono truncate flex-1">{formImage}</span>
                        <button onClick={() => setFormImage('')} className="text-rose-400 hover:text-rose-300 text-xs">✕</button>
                      </div>
                    )}
                  </div>
